...
class NBodySystem:
    ...
    def __init__(
        ...
        masses : (N,) array
        soft : softening length to avoid singularities
        """
    ...
    def _derivatives(self, t: float, y: np.ndarray) -> np.ndarray:
        """State derivative for ODE solver: y = [pos, vel] flattened."""
    ...
    def step(self, dt: float, method: str = "RK45") -> None:
        """Advance the system by dt using adaptive ODE integration."""
    ...
    def center_of_mass(self) -> Tuple[np.ndarray, np.ndarray]:
        """Return (COM position, COM velocity)."""
    ...
    def run(
        ...
        """Run the simulation for total_time, calling optional callback each step."""
    ...
    def snapshot(self) -> dict:
        """Return current state as a plain dict (serializable)."""
        return {
            "time": self.time,
...
def create_solar_system() -> NBodySystem:
    """
    Create a simplified Solar System (Sun + 8 planets).
    Units: AU, solar masses, years (G=1 calibrated).
    Data approximate, sufficient for stable integration.
    """
    ...
    # Approximate semi-major axes (AU) and circular velocities
    a = np.array([0.0, 0.387, 0.723, 1.0, 1.524, 5.203, 9.537, 19.191, 30.069])
    # v = sqrt(GM/a) with G=1, M=1 → v = 1/sqrt(a)
    v_circ = np.zeros_like(a)
    v_circ[1:] = 1.0 / np.sqrt(a[1:])

    positions = np.zeros((9, 3))
    positions[:, 0] = a  # all start on x-axis

    velocities = np.zeros((9, 3))
    velocities[:, 1] = v_circ  # circular motion in xy plane
...
if __name__ == "__main__":
    ...
    start = time.time()
    system.run(total_time=0.1, dt=0.002)  # ~36 days
